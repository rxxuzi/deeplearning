...
def get_dot_graph(output, verbose=True):
    ...
    Build a graph of functions and Vars backward-reachable from the
    output. To visualize a graphviz DOT text, you need the dot binary from the
    graphviz package (www.graphviz.org).

    Args:
        output (dezero.Var): Output Var from which the graph is
            constructed.
...
def reshape_sum_backward(gy, x_shape, axis, keepdims):
    ...
    Args:
        gy (dezero.Var): Gradient Var from the output by backprop.
    ...
    Returns:
        dezero.Var: Gradient Var which is reshaped appropriately
    """
    ndim = len(x_shape)
    tupled_axis = axis
    if axis is None:
...
def gradient_check(f, x, *args, rtol=1e-4, atol=1e-5, **kwargs):
    ...
    Args:
        f (callable): A function which gets `Var`s and returns `Var`s.
        x (`ndarray` or `dezero.Var`): A traget `Var` for computing
            the gradient.
        *args: If `f` needs Vars except `x`, you can specify with this
            argument.
        rtol (float): The relative tolerance parameter.
        atol (float): The absolute tolerance parameter.
        **kwargs: If `f` needs keyword Vars, you can specify with this
            argument.
...
def numerical_grad(f, x, *args, **kwargs):
    ...
    Args:
        f (callable): A function which gets `Var`s and returns `Var`s.
        x (`ndarray` or `dezero.Var`): A target `Var` for computing
            the gradient.
        *args: If `f` needs Vars except `x`, you can specify with this
            argument.
        **kwargs: If `f` needs keyword Vars, you can specify with this
    ...
    x = x.data if isinstance(x, Var) else x
    xp = cuda.get_array_module(x)
    if xp is not np:
    ...
    while not it.finished:
        ...
        if isinstance(y1, Var):
            y1 = y1.data
        y1 = y1.copy()

        x[idx] = tmp_val - eps
        y2 = f(x, *args, **kwargs)  # f(x-h)
        if isinstance(y2, Var):
            y2 = y2.data
        y2 = y2.copy()
...
def array_equal(a, b):
    ...
    Args:
        a, b (numpy.ndarray or cupy.ndarray or dezero.Var): input arrays
            to compare

    Returns:
        bool: True if the two arrays are equal.
    """
    a = a.data if isinstance(a, Var) else a
    b = b.data if isinstance(b, Var) else b
    a, b = cuda.as_numpy(a), cuda.as_numpy(b)
    return np.array_equal(a, b)


def array_allclose(a, b, rtol=1e-4, atol=1e-5):
    """Returns True if two arrays(or Vars) are element-wise equal within a
    tolerance.
